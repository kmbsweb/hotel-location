...
filter(osaka,yearclass=="2006年以前")
#MAC:family = "HiraKakuPro-W3"
A <- ggplot() +
  geom_sf(data=pref_27, fill = 'white') +
  geom_sf(data=railO, color="gray") +
  geom_point(data=osaka %>% filter(yearclass=="2013年以降") , aes(x=fX, y=fY),color="navy",size=0.7)+
  geom_text_repel(aes(x=135.2432,y=34.4361, label = "関西国際空港"),  size = 2.5)+
...
ggplot() +
  ...
  blank() +
  north(pref_27) +
  scalebar(pref_27, dist = 5, dd2km = TRUE, model = 'WGS84')

  
  scalebar(data=pref_27,st.size = 2,dist = 2, height = 0.008,dd2km = FALSE, model = "WGS84", location = "bottomleft")+
